...
# MusicBox 唱片播放器 v3.3.4 更新日志

## 更新日期
2026-01-20

## 新增 / 优化
- 🎵 **前端播放器全局状态统一**，`window.musicboxState` 保存当前播放列表、模式和索引，避免多模块状态混乱。
- 🔄 **修复随机播放重复问题**，`switchTo` 防止连续播放同一首歌。
- 🆕 **增量导入歌单**：导入时自动去重，避免重复歌曲。
- ✅ **一键检测有效性**：使用同一个 `songs` 列表，检测结果直接更新界面状态。
- 🧹 **删除失效歌曲**：支持一键清理所有无效歌曲。
- 💻 **Ajax接口增强**：单曲自动补全、歌单导入接口支持缓存、nonce校验及错误处理。
- 🖥️ **UI优化**：
  - 歌单编辑区可显示封面预览。
  - 播放器悬浮提示、滚轮调音量、双击切歌效果保持。
  - 拖拽支持，防误触点击。
- ⚡ **轻量修复**：加载歌曲速度优化，无需修改原逻辑，只调用唯一入口 `switchTo`。

## 已修复问题
- 修复随机播放时可能连续播放同一首歌的情况。
- 修复检测有效性时列表状态不同步的问题。
- 修复多模块同时调用 `loadSong` 导致状态错乱的问题。

## 说明
- 本次更新为灰度优化，**主要针对状态管理和播放逻辑稳定性**，原有功能保持兼容。
- 建议用户更新至本版本以避免多模块播放冲突。
...
- ✅ 修复「检测后删除失效歌曲无效」的问题  
- ✅ 统一后台歌曲状态源，避免 `_valid` 状态不同步  
- ✅ 检测结果可在刷新后正确恢复显示  
- ✅ 删除逻辑仅删除明确判定为失效的歌曲  
- ♻️ 后台 JS 结构整理，减少状态错乱风险  
